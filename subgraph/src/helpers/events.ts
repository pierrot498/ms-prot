...
export function recordNftEvent(
  ...
  dropAuction: AuctionPrimarySale | null = null
): void {
  ...
  if (dropAuction) {
    historicalEvent.dropAuction = dropAuction.id;
  }
...
export function removePreviousTransferEvent(event: ethereum.Event): void {
  ...
  for (
    let i = event.logIndex.minus(ONE_BIG_INT);
    i.ge(ZERO_BIG_INT);
    i = i.minus(ONE_BIG_INT)
  ) {
    let previousEvent = NftHistory.load(
      getPreviousEventId(event, "Transferred", i)
    );
    if (previousEvent) {
