...
async function main() {
  ...
  const updateFixedListing = await mkt.connect(addr2).updateListing(1, 1, 0, ethers.utils.parseEther("2"), currency, 0, 0);
  const updateFixedListingTx = await updateFixedListing.wait()
  console.log(updateFixedListingTx.events);

  const listing1 = await mkt.listings(2)
