...
async function main() {
  ...
  const buy1 = await mkt.connect(addr3).buy(2, addr3.address, 1, currency, ethers.utils.parseEther("1"), {value: ethers.utils.parseEther("1")});
  const buy1Tx = await buy1.wait()
  console.log(buy1Tx.events);
